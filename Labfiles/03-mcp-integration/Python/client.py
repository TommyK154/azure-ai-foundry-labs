...
async def chat_loop(session):
    ...
    with (
        ...
        function_dict = {tool.name: make_tool_func(tool.name) for tool in tools}
        ...
        agent = project_client.agents.create_version(
            ...
            definition=PromptAgentDefinition(
                ...
                - Recommend clearance if item inventory > 15 and weekly sales < 5
                """,
        ...
        while True:
            ...
            for item in response.output:
                if item.type == "function_call":
                    # Retrieve a matching function tool
                    function_name = item.name
                    kwargs = json.loads(item.arguments)
                    required_function = function_dict.get(function_name)
